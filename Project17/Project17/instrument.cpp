...
void Piano::metronome() {
	LPWSTR wide_str = multi_to_wide("C:\\Users\\Choi\\Downloads\\Metronome90.wav");
	PlaySound(wide_str, NULL, SND_FILENAME | SND_ASYNC | SND_LOOP); //일반 재생

}
string Piano::recordScale(char index) {
	switch (index) {
	case 'a'://도
		return "도";
		break;
...
void Piano::playInstrument(char index) {		//현재는 1~8온음,		이후 수정은 asdf jkl; 음계 wer uio 반음
...
void Guitar::playInstrument(char index) {		//현재는 1~8온음,		이후 수정은 asdf jkl; 음계 wer uio 반음
	LPWSTR wide_str = multi_to_wide(&soundVec.at(0)[0]);		// 초기화
